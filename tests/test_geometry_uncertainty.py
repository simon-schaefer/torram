...
def test_marginalize_distribution_stddev():
    ...
    stddev_new = torram.geometry.marginalize_distribution(cov, min_variance=1e-8, return_stddev=True)
    for i in range(4):
        assert torch.allclose(stddev_new[..., i], torch.sqrt(cov[..., i, i]))
...
def test_marginalize_distribution_diagonal():
    ...
    for i in range(4):
        assert torch.allclose(cov_new[..., i, i], torch.sqrt(cov[..., i, i]))
