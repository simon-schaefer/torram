...
def geodesic_loss(x_hat: torch.Tensor, x: torch.Tensor, eps: float = 1e-7) -> torch.Tensor:
    ...
    Args:
        ...
        x: target rotation matrix (B, 3, 3).
        eps: numeric eps.
    """
    if not x_hat.ndim == x.ndim == 3 or not x_hat.shape[-1] == x_hat.shape[-2] == x.shape[-1] == x.shape[-2] == 3:
